...
import { AuthenticationService, BulletinService, CommonFunctions } from 'app/shared/services';
...
export class BulletinComponent implements OnInit {
  ...
  onAddButtonClick() {
    ...
    this.chosenBulletin.clubName = this.currentUser.userClub;
    this.chosenBulletin.submitterUserName = this.currentUser.userName;
  }
  ...
  isUserSubmitter() {
    return this.currentUser.userRole == UserRoles.CLUB_ACCESS;
  }
  ...
  ngOnInit() {
    ...
    if (this.currentUser.userRole == UserRoles.CLUB_ACCESS) {
      // console.log('showing club data');

      this.bulletinService.getPendingBulletinsForSubmitter(this.currentUser.userClub).subscribe(res => {
        this.bulletinService.convertJdbcDatesToAngularInList(res);
      ...
      this.bulletinService.addNewBulletin(this.chosenBulletin).subscribe(res => {
        ...
        if (this.fileUploadedForPoster != null) {
          this.bulletinService.addOrUpdatePosterForBulletin(this.fileUploadedForPoster, this.chosenBulletin.bulletinTitle).subscribe(response => {
            ...
            this.messageService.add({ severity: 'success', summary: res.responseDetail + CommonFunctions.REFRESH_PAGE_MESSAGE });
